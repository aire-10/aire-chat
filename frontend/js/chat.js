document.addEventListener("DOMContentLoaded", () => {
  ...
  async function syncFromApi() {
    ...
    if (Array.isArray(sessions) && sessions.length > 0) {
      ...
      const sessionToLoad = activeId || sessions[0].id;
      if (sessionToLoad) {
        const messages = await apiRequest(`/sessions/${sessionToLoad}/messages`);
        if (Array.isArray(messages)) {
          localStorage.setItem(sessionKey(sessionToLoad), JSON.stringify(messages));
        }
      }
    }
  }
  ...
  async function saveSessions(sessions) {
    localStorage.setItem(STORAGE_SESSIONS, JSON.stringify(sessions));
    await apiRequest('/sessions', 'PUT', { id: sessions[0]?.id ?? '', title: sessions[0]?.title ?? 'New Chat' });
    // also update each session in API
    for (const session of sessions) {
      await apiRequest(`/sessions/${session.id}`, 'PUT', session);
    }
  }
  ...
  async function saveSessionMessages(id, messages) {
    ...
    await apiRequest(`/sessions/${id}/messages`, 'PUT', { messages, title: loadSessions().find(s => s.id === id)?.title });
  }
  ...
  const addMessage = (role, text, ts = Date.now()) => {
    ...
    messages.push(msg);
    saveSessionMessages(activeId, messages).catch(() => {});
    touchSession(activeId);
    renderMessage(msg);
  };
